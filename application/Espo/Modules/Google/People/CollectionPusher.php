...
/**
 * Creates new contacts if pair does not exists in a batch create request.
...
class CollectionPusher
{
    ...
    private $ownEmailAddressFetcherFactory;
    ...
    public function push(Collection $collection, CollectionPusherParams $params): PushResult
    {
        ...
        if (count($createContactList)) {
            $createdCount = $this->processCreateBatchRequest($createContactList, $contactEntityMap, $params);
        }
    ...
    private function createContactFromEntity(
        ...
        $emailAddressData = $this->entityManager->getRepository('EmailAddress')->getEmailAddressData($entity);
        $phoneNumberData = $this->entityManager->getRepository('PhoneNumber')->getPhoneNumberData($entity);
    ...
    private function processCreateBatchRequest(
        ...
        foreach ($list as $i => $item) {
            ...
            if (!$entity) {
                throw new RuntimeError("No contact entity in map.");
            }
    ...
    private function findPair(Entity $entity): ?Pair
    {
        ...
        $pair = $this->entityManager
            ->getRepository('GoogleContactsPair')
            ->where([
    ...
    {
        $externalAccount = $this->entityManager->getEntity('ExternalAccount', 'Google__' . $this->userId);
