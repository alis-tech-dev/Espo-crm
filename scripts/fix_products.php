...
$entities = [
    'Product'  // Focus only on Lead entities
];
            ...
            try {
                ...
                if (empty($parsedBody->list)) {
                    ...
                    foreach ($parsedBody->list as $item) {
                        ...
                        $collection = $entityManager->getRDBRepository('Product')->where('productCode', $item->productCode)->find();

                        if (count($collection) == 1) {
                            $existingEntity = $collection[0];
                        } else {
                            continue;
                        }
                        ...
                        if ($existingEntity) {
                            try {
                                $existingEntity->set( [
                                    'priceJesenoConverted' => $item->priceJesenoConverted,
                                    'priceEndUser' => $item->priceE,
                                    'unit' => $item->unit
                                ]);
