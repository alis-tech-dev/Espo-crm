                    ...
                    foreach ($parsedBody->list as $item) {
                        ...
                        if ($existingEntity) {
                            try {
                                ...
                                // Fetch and relate product databases
                                $productDatabasesResponse = $client->request(Client::METHOD_GET, "BusinessProject/{$entityId}/productDatabases1", []);
                                $productDatabases = $productDatabasesResponse->getParsedBody()->list;

                                if (!empty($productDatabases)) {
                                    foreach ($productDatabases as $productDatabase) {
                                        $productDatabaseEntity = $entityManager->getEntityById('ProductDatabase', $productDatabase->id);

                                        if (!$productDatabaseEntity) {
                                            $productDatabaseEntity = $entityManager->createEntity('ProductDatabase', (array)$productDatabase);
                                        }

                                        $entityManager->getRDBRepository('SalesOrder')
                                        ->getRelation($existingEntity, 'productDatabases')
                                        ->relate($productDatabaseEntity);
                                    }
                                }
